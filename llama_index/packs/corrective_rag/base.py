...
#Change def __init__(self, vector_store: List[Document], tavily_ai_apikey: str) -> None: add LLM
    #Online Search:
    #def __init__(self, vector_store: List[Document], tavily_ai_apikey: str, llm, path: str) -> None:
    #Offline Search:
    def __init__(self, vector_store: List[Document], llm, path: str) -> None:
        self.llm = llm
        self.relevancy_pipeline = QueryPipeline(
            chain=[DEFAULT_RELEVANCY_PROMPT_TEMPLATE, self.llm]
        )
        self.transform_query_pipeline = QueryPipeline(
            chain=[DEFAULT_TRANSFORM_QUERY_TEMPLATE, self.llm]
        )
#Change self.index = VectorStoreIndex.from_documents(documents)
        self.index = VectorStoreIndex.from_vector_store(vector_store)
        #Online Search:
        #self.tavily_tool = TavilyToolSpec(api_key=tavily_ai_apikey)
        #Offline Search:
        self.whoosh = open_dir(path)
    ...
    """
    #Offline Search:
    def search_index(self, query_str: str):
        with self.whoosh.searcher() as searcher:
            # Use a parser to parse the query
            query = QueryParser("content", self.whoosh.schema).parse(query_str)
    ...
    def run(self, query_str: str, **kwargs: Any) -> Any:
        ...
        if "no" in relevancy_results:
            ...
            search_text = self.search_index(transformed_query_str)

            #Offline Search:
            #search_text = self.search_local_html_files(transformed_query_str)
